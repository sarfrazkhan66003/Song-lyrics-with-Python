# Wishes Song — Lyrics Typewriter Player
...
## Run
```bash
.venv/bin/python song_with_lyrics.py
```
...
## Notes
- Media files like `song.mp3` are excluded via so the repo stays lightweight.
- You can change the YouTube URL or the start time in `song_with_lyrics.py`.
- If you want to include the audio in the repo, remove the relevant patterns.
